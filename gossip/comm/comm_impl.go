...
func NewCommInstance(s *grpc.Server, cert *csptls.Certificate, idStore identity.Mapper,
...
func createGRPCLayer(port int) (*grpc.Server, net.Listener, api.PeerSecureDialOpts, []byte) {
	...
	tlsConf := &csptls.Config{
		Certificates:       []csptls.Certificate{cert},
		ClientAuth:         csptls.RequestClientCert,
		InsecureSkipVerify: true,
	}
	serverOpts = append(serverOpts, grpc.Creds(credentials.NewTLS(tlsConf)))
	ta := credentials.NewTLS(&csptls.Config{
		Certificates:       []csptls.Certificate{cert},
		InsecureSkipVerify: true,
	})
