...
type Options struct {
	...
	// server-only option
	ClientAuth csptls.ClientAuthType
...
var acceptedCBCCiphers = []uint16{
	csptls.TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA,
	csptls.TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA,
	csptls.TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA,
	csptls.TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA,
	csptls.TLS_RSA_WITH_AES_256_CBC_SHA,
	csptls.TLS_RSA_WITH_AES_128_CBC_SHA,
}
...
func ServerDefault() *csptls.Config {
	return &csptls.Config{
		// Avoid fallback to SSL protocols < TLS1.0
		MinVersion:               csptls.VersionTLS10,
...
func ClientDefault() *csptls.Config {
	return &csptls.Config{
		// Prefer TLS1.2 as the client minimum
		MinVersion:   csptls.VersionTLS12,
		CipherSuites: clientCipherSuites,
	}
}

// certPool returns an X.509 certificate pool from `caFile`, the certificate file.
func certPool(caFile string, exclusivePool bool) (*cspx509.CertPool, error) {
	// If we should verify the server, we need to load a trusted ca
	var (
		certPool *cspx509.CertPool
		err      error
	)
	if exclusivePool {
...
func adjustMinVersion(options Options, config *csptls.Config) error {
...
// decrypt it.
func getCert(options Options) ([]csptls.Certificate, error) {
	...
	tlsCert, err := csptls.X509KeyPair(cert, prKeyBytes)
	if err != nil {
		return nil, errors.Wrap(err, errMessage)
	}

	return []csptls.Certificate{tlsCert}, nil
}

// Client returns a TLS configuration meant to be used by a client.
func Client(options Options) (*csptls.Config, error) {
...
func Server(options Options) (*csptls.Config, error) {
	tlsConfig := ServerDefault()
	tlsConfig.ClientAuth = options.ClientAuth
	tlsCert, err := csptls.LoadX509KeyPair(options.CertFile, options.KeyFile)
	if err != nil {
	...
	tlsConfig.Certificates = []csptls.Certificate{tlsCert}
	if options.ClientAuth >= csptls.VerifyClientCertIfGiven && options.CAFile != "" {
		CAs, err := certPool(options.CAFile, options.ExclusiveRootPools)
		if err != nil {
			return nil, err
		}
