...
type GRPCServer interface {
	...
	ServerCertificate() csptls.Certificate
...
type grpcServerImpl struct {
	...
	//the TLS handshake
	serverRootCAs []csptls.Certificate
...
func NewGRPCServerFromListener(listener net.Listener, secureConfig SecureServerConfig) (GRPCServer, error) {
	...
	if secureConfig.UseTLS {
		...
		if secureConfig.ServerKey != nil && secureConfig.ServerCertificate != nil {
			...
			cert, err := csptls.X509KeyPair(secureConfig.ServerCertificate, secureConfig.ServerKey)
			if err != nil {
				return nil, err
			}
			...
			certificates := []csptls.Certificate{grpcServer.serverCertificate}
			grpcServer.tlsConfig = &csptls.Config{
				Certificates:           certificates,
				SessionTicketsDisabled: true,
			}
			grpcServer.tlsConfig.ClientAuth = csptls.RequestClientCert
			//checkif client authentication is required
			if secureConfig.RequireClientCert {
				//require TLS client auth
				grpcServer.tlsConfig.ClientAuth = csptls.RequireAndVerifyClientCert
				//if we have client root CAs, create a certPool
				if len(secureConfig.ClientRootCAs) > 0 {
					grpcServer.clientRootCAs = make(map[string]*x509.Certificate)
					grpcServer.tlsConfig.ClientCAs = cspx509.NewCertPool()
...
//ServerCertificate returns the tls.Certificate used by the grpc.Server
func (gServer *grpcServerImpl) ServerCertificate() csptls.Certificate {
