...
func (cas *CASupport) GetDeliverServiceCredentials(channelID string) (credentials.TransportCredentials, error) {
	...
	var tlsConfig = &csptls.Config{}
	var certPool = cspx509.NewCertPool()
...
func (cas *CASupport) GetPeerCredentials(tlsCert csptls.Certificate) credentials.TransportCredentials {
	var creds credentials.TransportCredentials
	var tlsConfig = &csptls.Config{
		Certificates: []csptls.Certificate{tlsCert},
	}
	var certPool = cspx509.NewCertPool()
