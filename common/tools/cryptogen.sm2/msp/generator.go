...
func GenerateLocalMSP(baseDir, name string, sans []string, signCA *ca.CA,
	...
	*/
	...
	// generate sm2 certificate using signing CA
	cert, err := signCA.SignCertificate(filepath.Join(mspDir, "signcerts"),
		name, []string{}, ecPubKey, sm2.KeyUsageDigitalSignature, []sm2.ExtKeyUsage{})
	if err != nil {
		return err
	}
	...
	err = x509Export(filepath.Join(mspDir, "cacerts", sm2Filename(signCA.Name)), signCA.SignCert)
	if err != nil {
		return err
	}
	// the TLS CA certificate goes into tlscacerts
	err = tlsx509Export(filepath.Join(mspDir, "tlscacerts", sm2Filename(tlsCA.Name)), tlsCA.SignCert)
	if err != nil {
		return err
	}
	...
	// of unit tests
	err = x509Export(filepath.Join(mspDir, "admincerts", sm2Filename(name)), cert)
	if err != nil {
		return err
	}
	...
	*/
	...
	// generate sm2 certificate using TLS CA
	...
	// rename the generated TLS sm2 cert
	err = os.Rename(filepath.Join(tlsDir, sm2Filename(name)),
...
func GenerateVerifyingMSP(baseDir string, signCA *ca.CA, tlsCA *ca.TlsCA) error {
	...
	if err == nil {
		// the signing CA certificate goes into cacerts
		err = x509Export(filepath.Join(baseDir, "cacerts", sm2Filename(signCA.Name)), signCA.SignCert)
		if err != nil {
			return err
		}
		// the TLS CA certificate goes into tlscacerts
		err = tlsx509Export(filepath.Join(baseDir, "tlscacerts", sm2Filename(tlsCA.Name)), tlsCA.SignCert)
		if err != nil {
			return err
		}
	}
	...
	_, err = signCA.SignCertificate(filepath.Join(baseDir, "admincerts"), signCA.Name,
		[]string{""}, ecPubKey, sm2.KeyUsageDigitalSignature, []sm2.ExtKeyUsage{})
	if err != nil {
		return err
	}
...
func sm2Filename(name string) string {
