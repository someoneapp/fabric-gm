	...
	if err == nil {
		...
		if err == nil {
			...
			if err == nil {
				...
				//TODO TODO TODO !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
				sm2Cert, err := genCertificateSM2(baseDir, name, &template, &template,
					ecPubKey, signer)
				response = err
				if err == nil {
					ca = &CA{
...
func (ca *CA) SignCertificate(baseDir, name string, sans []string, pub *sm2.PublicKey,
	...
	cert, err := genCertificateSM2(baseDir, name, &template, ca.SignCert,
		pub, ca.Signer)
...
// default template for sm2 certificates
...
// generate a signed sm2 certficate using SM2
func genCertificateSM2(baseDir, name string, template, parent *sm2.Certificate, pub *sm2.PublicKey,
	...
	sm2Cert, err := sm2.ParseCertificate(certBytes)
	if err != nil {
		return nil, err
	}
	return sm2Cert, nil
...
// SignCertificate creates a signed certificate based on a built-in template
// and saves it in baseDir/name
func (ca *TlsCA) SignCertificate(baseDir, name string, sans []string, pub *ecdsa.PublicKey,
